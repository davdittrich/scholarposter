...
my_etiquette = Etiquette('mastodon link exporter', '0.1', 'https://economicscience.net/', 'dd-crossred@dr-dittrich.de')
...
message = "#EconSky\n" + (message) if  re.search(r"[E|e]con|[M|m]arket|[W|w]age|[G|g]ameTheory|[L|l]abor|[B|b]ounded[R|r]ationality", message) else message
message = (message) + "\n\norig " + str(tweet.url) + " "
...
BLUESKY_EMAIL = os.environ["BLUESKY_EMAIL"]
...
def fetch_embed_url_card(url: str) -> Dict:
  ...
  url = unshorten_url(url) if not bool(re.search(r"doi",url)) else url
  urldesc = url if bool(re.search(r"repec",url)) else ""
...
#    url = unshorten_url(url)
    scraper =  cloudscraper.create_scraper(delay=10, browser={
        'custom': 'SocialMedia-OG-Parser/1.0'})
    reqs = None
    try:
      reqs = scraper.get(url)
    except:
      print("Problems getting html 1\n")
      try:  
        reqs = requests.get(url, verify=False, timeout=10)
      except Exception as e:
        fout = open('mastodon2bluesky.log', 'a')
        outj = f"Problems getting html: {url} : {e}"
        print(outj, "\n")
        out = ' '.join([datetime.now().strftime("%m/%d/%Y, %H:%M:%S"), str(outj), str(tweet.id), "\n"])
        fout.write(out)
        fout.close()

    soup = BeautifulSoup(reqs.text, 'lxml') if reqs else None     
    title_tag = soup.find("meta", property="og:title") if soup else None
    if title_tag:
      card["title"] = title_tag["content"]
    else:
      try:  
       card["title"] = html.unescape(_get_title(soup)) if soup else None
      except:
        print("No title found\n")

    description_tag = soup.find("meta", property="og:description") if soup else None
    if description_tag: 
    ...
    image_tag = soup.find("meta", property="og:image") if soup else None
    if image_tag:
      img_url = image_tag["content"]
      if "://" not in img_url:
          img_url = url + img_url
      img_url = "https://info.arxiv.org/brand/images/brand-logo-primary.jpg" if "arxiv-logo-fb.png" in img_url else img_url   
      ...
      fout = open('mastodon2bluesky.log', 'a')
      outj = f"no preview image at: {url}"
      print(outj, "\n")
      out = ' '.join([datetime.now().strftime("%m/%d/%Y, %H:%M:%S"), str(outj), str(tweet.id), "\n"])
      fout.write(out)
      fout.close()
...
#   exit()
    ...
    if 'pdf' in ltype:
      try:
        scraper =  cloudscraper.create_scraper(delay=10, browser={
        'custom': 'SocialMedia-PDF-Parser/1.0'})
        reqs = scraper.get(url)
      except:
        reqs = requests.get(url, verify=False, timeout=10)   
      try:
...
#  if bool(re.search(r"doi",url)):
  try:
    udoi = None
    udoi_list = re.findall(r'\b10\.\d{4,9}/[-.;()/:\w]+', url)
    if len(udoi_list) > 0:
      udoi = udoi_list[0]
    if udoi :
        print("DOI: ", udoi, "\n")      
        ...
        out = works.doi(udoi)
        card["title"] = out["title"][0] if out["title"][0] is not None else card["title"]
        card["description"] = BeautifulSoup(out["abstract"], 'lxml').getText().strip() if out["abstract"] is not None else card["description"]
  except:
    print("DOI Error 2\n")

  card["title"] = card["title"] if card["title"] is not None else ''
...
#                  w, h = img.size  
#                  print(f_name, "\nSize: ", w, ":", h, "\n")
                  img.save(im_out, format="PNG", optimize=True)
                  content = im_out.getvalue()
                  if len(content) > 999997:
                    im_out = io.BytesIO()
                    img = None                    
                    img = Image.open(f).convert('RGB')
                    if img is None:
                      print("Error: png2JPEG 1")
   #                  w, h = img.size  
   #                  print(f_name, "\nSize: ", w, ":", h, "\n")
                    img.save(im_out, format="JPEG", optimize=True)
                else:
...
#                  print(alt_text,"\n")
    ...
    if images:
      ...
      print("no image!\n")  
      urls = None
      urls = re.search(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', str(text))
      if urls:
    ...
    text = text[0:299] if len(text) > 300 else text
...
def reply_to_bluesky(text, reply):
    ...
    text = text[0:299] if len(text) > 299 else text
...
# check length
if tweet_length < 298:
  tweet_length_limit = 298

elif tweet_length >= 298:
  ...
  if re.search(r'\S\.\S', message):
    if re.search(r'http', message):
      tweet_length_limit = 292
    else:
      tweet_length_limit = 287
  else:
    tweet_length_limit = 292
