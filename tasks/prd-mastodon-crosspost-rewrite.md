...
- [ ] Idempotency: **key-level raw text scan** (FR-107). Before appending a key, the raw file is scanned for a commented-out line matching `# key =`. If found, the key is skipped. This is key-level — a section that received some keys in v1.2.0 will still receive newly-added keys in v1.3.0. Active (uncommented) keys are already excluded by the TOML-parse diff in step 1.
...
- [ ] All output from `--dry-run` and `--diff` is passed through TOML-aware redaction before printing: any line where the key matches `_SENSITIVE_FIELDS` (defined in `cli.py`) has its value replaced with `<redacted>`. TOML syntax is `key = "value"` (not URL-encoded), so the redaction pattern must match `key\s*=\s*"[^"]*"` form, not the URL-encoded form used elsewhere.
- [ ] File write uses atomic replacement: write updated content to a `.tmp` file then `os.rename()` to the target path — directly adapting `_atomic_write()` in `state.py` (which already uses text mode via `os.fdopen(fd, "w")`; no binary-mode deviation needed).
...
| FR-103 | **Append-only diff algorithm**: (1) Parse both files with `tomllib` to produce candidate list: all top-level sections and leaf keys present in the example but absent from the user file. (2) Apply key-level idempotency filter (FR-107) — remove any key already present as a commented-out line in the raw file. (3) Append remaining candidate keys at EOF. **Section header rules**: for entirely absent sections, prepend a commented-out `# [section]` header before the keys. For sections that already exist in the user file (only leaf keys missing), do NOT add a `# [section]` header — append only the missing key lines to avoid creating a duplicate table header that would cause a TOML parse error if uncommented. Ensure exactly one blank line between existing file content and the first appended block. Handles arbitrarily nested sub-tables by appending full subtable blocks. |
| FR-104 | Comment format: each appended block is preceded by `# Added by scholarposter config-update {version} — {section}` (informational only, not used for idempotency). Each appended key line is prefixed `# `. Version from `importlib.metadata.version("scholarposter")`, fallback `"dev"`. |
...
| FR-106 | `--dry-run`: prints appended lines (TOML-aware redacted) to stdout, exits 0, writes nothing. `--diff`: prints unified diff (redacted) to stdout, exits 0, writes nothing. Both flags together: equivalent to `--diff`. Redaction: lines where the key matches `_SENSITIVE_FIELDS` have their value replaced with `<redacted>` using a TOML-syntax pattern (`key\s*=\s*"[^"]*"`), not URL-encoded form. |
| FR-107 | **Key-level idempotency via raw-text scan**: for each candidate key `k`, scan the raw text of `config.toml` for a commented-out line matching `# k =` (prefix match). If found, skip that key — it was appended by a prior run. This is key-level, not section-level: a section that already received some appended keys in v1.2.0 will correctly receive newly-added keys in v1.3.0. No section-level sentinel; no version pinning. |
...
- **Key-level idempotency (no sentinel)**: `tomllib` won't see commented-out keys, so TOML-parse cannot detect prior runs. A section-level sentinel was considered but rejected: it would permanently block future key additions to already-updated sections. Key-level raw text scan (`# key =` prefix match) is both simpler and correct — each key is individually guarded across all past and future versions.
...
- **Section header omitted for existing sections**: when appending leaf keys to a section that already exists in the user file, no `# [section]` header is appended. Adding one would create a duplicate table definition — a fatal TOML parse error if the user uncomments the key. Only entirely absent sections receive a commented-out header.
...
| T-36 | **Idempotency**: run twice on a config missing `[audit]`; first run appends commented-out keys; second run detects them via key-level scan and makes no further changes | Unit |
...
| T-42 | **Cross-version idempotency**: v1.2.0 appended `key_a` to `[audit]`; re-run under v1.3.0 with no new keys — file byte-identical, `key_a` not duplicated | Unit |
| T-43 | **Cross-version new key**: v1.2.0 appended `key_a` to `[audit]`; v1.3.0 adds `key_b` to `[audit]`; only `key_b` is appended; `key_a` is not duplicated | Unit |
| T-44 | **Sensitive value redaction**: example contains `smtp_password = "changeme"`; `--dry-run` output shows `smtp_password = <redacted>` | Unit |
